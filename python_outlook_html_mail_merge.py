...
file = pd.ExcelFile('ExcelWorkbookTemplate.xlsx') #Establishes the excel file you wish to import into Pandas
...
for index, row in df.iterrows(): #Loops through each row in the dataframe
    ...
    status = str((row['Status'])) #Set a status to skip over this


    if (pd.isnull(status) or pd.isnull(email) or pd.isnull(subject) or pd.isnull(body)): #Skips over rows where one of the cells in the three main columns is blank
        continue
